<template>
  <div class="m-dashboard m-dashboard-work m-dashboard-other">
    <el-tabs v-model="searchType">
      <el-tab-pane :label="item" :name="key" v-for="(item, key) in types" :key="key"> </el-tab-pane>
    </el-tabs>
    ...
    <div class="m-dashboard-box" v-loading="loading">
      <template v-if="data && data.length">
        <collection class="m-dashboard-box-list" :data="data" v-if="searchType === 'collection'" />
        <item_plan class="m-dashboard-box-list" :data="data" v-if="searchType === 'item_plan'" @refresh="loadPosts(searchType)" />
        <question class="m-dashboard-box-list" :data="data" v-if="searchType === 'question'" />
        <paper class="m-dashboard-box-list" :data="data" v-if="searchType === 'paper'" />
        <namespace class="m-dashboard-box-list" :data="data" v-if="searchType === 'namespace'" />
      </template>
      <el-alert v-else class="m-dashboard-box-null" title="没有找到相关条目" type="info" center show-icon> </el-alert>
      <el-pagination class="m-dashboard-box-pages" background :page-size="per" :hide-on-single-page="true" :current-page.sync="page" layout="total, prev, pager, next, jumper" :total="total"> </el-pagination>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    params: function() {
      ...
      } else if (this.searchType == 'item_plan' || this.searchType == 'collection' || this.searchType == 'namespace') {
        return {
          page: this.page,
          keyword: this.search,
          limit: this.per,
  ...
  methods: {
    loadPosts: function(searchType) {
      ...
      fn[searchType](this.params)
        .then((res) => {
          if (searchType == 'item_plan' || searchType == 'collection' || searchType == 'namespace') {
            res = res.data
            if (res.code === 200) {
